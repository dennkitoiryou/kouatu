...
## 基本統計量の確認（終値）
print('終値の基本統計量kihonntoukeiryou')
print(df['kennsuu'].describe(), '\n')

## 終値を時系列にプロット
plt.title('日経平均株価（終値）の推移 nikkeiheikinnkabukanosuii')
plt.plot(range(len(closing_price)), closing_price)
plt.show()

plt.title('日経平均株価（終値）の推移　過去300日分のみ nikkeiheikinnkabuka nosuii kako300nitibunn')
plt.plot(range(len(closing_price[-300:])), closing_price[-300:])
plt.show()

plt.title('日経平均株価（終値）の推移　過去30日分のみnikkeiheikinnkabuka nosuii kako130nitibunn')
...
(X_valid, y_valid) = data_split(closing_price, -160, -30, lookback)
...
plt.title("実際の終値と予測値jissaino owarine to yosokuti")
plt.plot(pre_date, y_test, 'b', alpha=0.6, marker='.', label="実際の終値jissaino owarine", linewidth=1)
